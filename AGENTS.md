...
- 머지가 끝난 워크트리·브랜치는 `scripts/prune_merged_worktrees.sh`(SessionStart hook)가 지운다. squash 때문에 `git branch --merged main`은 여기서 아무것도 찾지 못하므로 판정은 `gh`에 묻는다 — 그 판정 경로나 가드 여섯(범위·자기 자신·locked·detached·미커밋 변경·tip==`headRefOid`)을 느슨하게 하지 말 것.
