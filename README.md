...
- **성별·탐구 과목 문항은 하드 필터**: 미충족 대학(예: 남학생→여대, 사탐 2과목→고려대(세종) 약대)은 득표와 무관하게 추천에서 제외되고 "지원 불가"로 표시. 결과 화면은 임계값별 제외 사유(`filterLabels`)로 묶어 보여준다. 어떤 문항이 하드 필터인지와 사유 라벨은 `scripts/convert_criteria.py`의 `HARD_FILTER_QUESTIONS`가 정하고, `src/data/criteria/*.json`의 `hardFilter`/`filterLabels`로 내려온다
- 탐구 문항은 3지선다(과탐 2과목 / 과탐 1과목+사탐 1과목 / 사탐 2과목). 기준표 셀은 대학의 최저학력기준을 따른다 — 과(2) 요구 `1▲`, 과(1) 요구 `2▲`, 탐구 무관 `3▲`
- 동점 1위는 공동 1위로 모두 표시, 순서는 기준표 컬럼 순서 유지

## 결과 화면 보조 데이터 (강사 강의자료 전사본)

`src/data/admission-info.ts`(전형·수능최저·고사일정 표), `src/data/exam-schedule.ts`(대학별 고사 일시·수능 전 여부), `src/data/exam-scope.ts`(대학별 수학 출제 범위)는 강사의 2027 원서지원 전략 강의자료(`의치한연고 송출용.pdf`)를 옮겨 적은 것이다. 결과 화면은 이 데이터로 랭킹 행에 고사일·"수능 전" 배지와 출제 범위를 붙이고, 추천 후보끼리 같은 날 같은 시간대에 시험을 보면 "같은 시간대에 시험 보는 대학"으로 묶어 경고한다(`src/lib/schedule.ts`). 대학 키는 기준표의 대학명과 같아야 하며, `schedule.test.ts`가 누락을 잡는다. 강의자료가 갱신되면 이 세 파일만 고치면 된다.
