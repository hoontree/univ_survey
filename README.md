...
- **성별·과탐 2과목 문항은 하드 필터**: 미충족 대학(예: 남학생→여대, 과탐 미응시→고려대(세종) 약대)은 득표와 무관하게 추천에서 제외되고 "지원 불가"로 표시. 결과 화면은 제외 사유(`filterLabel`)별로 묶어 보여준다. 어떤 문항이 하드 필터인지는 `scripts/convert_criteria.py`의 `HARD_FILTER_QUESTIONS`가 정하고, `src/data/criteria/*.json`의 `hardFilter`/`filterLabel`로 내려온다. 기준표 셀이 `1▲`(네만 통과)인 대학이 곧 그 조건이 필수인 대학이다
